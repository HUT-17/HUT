...
CMultipleCameraDlg::CMultipleCameraDlg(CWnd* pParent /*=NULL*/)
    ...
    , m_nUsingCameraNum(-1)
{
	//{{AFX_DATA_INIT(CMultipleCameraDlg)
...
void CMultipleCameraDlg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Radio(pDX, IDC_CONTINUS_MODE_RADIO, m_bContinusModeRadio);//控件要加上组的属性，否则奔溃
    DDX_Radio(pDX, IDC_TRIGGER_MODE_RADIO, m_bTriggerModeRadio);//控件要加上组的属性，否则奔溃
    ...
    DDX_Text(pDX, IDC_FRAME_COUNT4_EDIT, m_nFrameCount4Edit);
...
BEGIN_MESSAGE_MAP(CMultipleCameraDlg, CDialog)
	//{{AFX_MSG_MAP(CMultipleCameraDlg)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
    ON_WM_TIMER()
	ON_BN_CLICKED(IDC_INIT_DEVICE_BUTTON, OnBnClickedInitDeviceButton)
	ON_BN_CLICKED(IDC_SET_EXPOUSE_GAIN_BUTTON, OnBnClickedSetExpouseGainButton)
	...
	ON_BN_CLICKED(IDC_SOFTWARE_MODE_BUTTON, OnBnClickedSoftwareModeButton)
	ON_BN_CLICKED(IDC_HARDWARE_MODE_BUTTON, OnBnClickedHardwareModeButton)
	ON_BN_CLICKED(IDC_SOFTWARE_ONCE_BUTTON, OnBnClickedSoftwareOnceButton)
	ON_BN_CLICKED(IDC_CLOSE_BUTTON, OnBnClickedCloseButton)
	ON_BN_CLICKED(IDC_CLEAR_COUNT_BUTTON, OnBnClickedClearCountButton)
    ON_WM_CLOSE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
int CMultipleCameraDlg::EnableControls(BOOL bIsCameraReady)
{
    ...
    if (!m_bOpenDevice)
    {
        ...
        GetDlgItem(IDC_FRAME_COUNT4_EDIT)->EnableWindow(FALSE);
...
int CMultipleCameraDlg::StartGrabbing(void)
{
    ...
    for (i = 0; i < m_nSelectDeviceNum; i++)
    {
        ...
        m_nUsingCameraNum = i;
        m_hGetOneFrameHandle[i] = (void*) CreateThread( NULL , 0 , WorkThread , this, 0 , NULL );
...
int CMultipleCameraDlg::StopGrabbing(void)
{
    ...
    for (int j = 0; j < m_nSelectDeviceNum; j++)
    {
        if (m_hGetOneFrameHandle[j])
        {
            CloseHandle(m_hGetOneFrameHandle[j]);
            m_hGetOneFrameHandle[j] = NULL;
        }
    }
    return nRet;
}
